...
- Identity is mapped from token claims (`BaseClaimIdentityMap`), never from credentials or api keys, so rotation cannot change the actor (CHR-ID-007). The map returns the corresponding keel grant principal: a human uses its claimed keel user id, while an agent uses `AgentPrincipalKind`, its exact stable Charter `AgentIdentity` id, and the authenticated `Session.PartnerID` as its sole `tenant_id` scope. This is the contract for a downstream's agent grant source. `Session` exposes token scopes for information, but they do not grant Charter authority.
...
`go.mod` gains four modules. Nothing is requested from keel: tenant scoping on change rows arrived with keel v1.2.55. Compatibility with v1.2.62 moves the OAuth token identity used by `Session` and `JobContext` from `port.Principal` to `model.TokenPrincipal`, and changes `PermissionChecker` plus `IdentityMap` to pass the separate grant-evaluation `model.Principal`. Agent actors can therefore use their own tenant-scoped keel grant source without a synthetic user id; the grant row's `agent_id` is the stable Charter identity id. Two generic pieces stay as upstream candidates once a project proves their shape: a pre-action port inside keel's generated CRUD and a queryable `port.TableLogger` destination (keel's filesystem logger is write-only, so `TableLogStore` refuses it). Scout composes these adapters; Charter ships no HTTP surface itself.
