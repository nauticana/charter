...
- `IdentityMap` connects keel subjects to Charter identities and maps an actor to the keel grant principal evaluated for it. `BaseClaimIdentityMap` reads identity claims, maps humans to `model.UserPrincipal`, and maps agents directly to the `agent` principal kind with their stable Charter id; `Caller` resolves the acting identity active at the action time.
...
- `PermissionGate` is an `authority.Evaluator` that layers keel's principal-aware `CheckActionPermission` behind a Charter evaluator: keel may deny, never widen, and unmapped capabilities fail closed. A custom `IdentityMap` may populate `model.Principal.Scope` when its registered keel grant source has extra scope filters.
